...
for(s in 1:length(species_codes)){
  ...
  writeRaster(species_stack, file = paste0("D:/floating_solar/generated/",sp,"_weekly_values.tif"), overwrite=TRUE)
...
for(a in 1:length(species_codes)){
  ...
  lake_bird_data_long <- pivot_longer(lake_bird_data,cols = !Water_ID, names_to="week_date")
  lake_bird_data_long$species_code <- rep(sp, length(lake_bird_data_long$Water_ID))
  lake_bird_data_long$week <- rep(1:52,length(lake_bird_data_long$Water_ID)/52)
  
  
  save(lake_bird_data, file = paste0("D:/floating_solar/data_outputs/",buf,"_",sp,"_lake_abd_weight_weekly.RData"))
